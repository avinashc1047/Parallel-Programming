...
int main(int argc, char *argv[]) {
    ...
    /* Fix seed function to distribute unique random seeds to all processes */
    int seeds[comm_sz];
    int my_seed;
    if (my_rank == 0) {
        // Master process generates unique seeds for each process
        srand(time(NULL));
    ...
    // Scatter the seeds to all processes, including master
    MPI_Scatter(seeds, 1, MPI_INT, &my_seed, 1, MPI_INT, 0, MPI_COMM_WORLD);
    
    // Each process sets its own random seed
    srand(my_seed);
    ...
    if (my_rank == 0) {
        ...
        printf("Serial Pi calculation: pi=%f (error=%f)\n", pi, error);
